...
export type ProjectDocument = { path: string; name: string; section: string; size: number; updatedAt: string };
export async function getJobExecutions(slug:string,id:string) { await getJob(slug,id); return json<{schemaVersion:1;executions:JobExecutionLink[]}>(await bounded(slug,"jobs",id,"executions.json")); }
export async function getJobHistory(slug:string,id:string):Promise<JobHistoryEvent[]> { await getJob(slug,id); const content=await readFile(await bounded(slug,"jobs",id,"history.jsonl"),"utf8"); return content.split("\n").filter(Boolean).map(line=>JSON.parse(line) as JobHistoryEvent); }
export async function listProjectDocuments(slug:string):Promise<ProjectDocument[]> { projectSlugSchema.parse(slug); const base=await bounded(slug),out:ProjectDocument[]=[]; async function walk(dir:string,prefix:string,depth:number){if(depth>4)return;for(const entry of await readdir(dir,{withFileTypes:true})){if(entry.name.startsWith(".")||entry.name==="node_modules"||entry.name==="attachments")continue;const path=join(dir,entry.name),rel=prefix?`${prefix}/${entry.name}`:entry.name;if(entry.isDirectory())await walk(path,rel,depth+1);else if(/\.(md|txt)$/i.test(entry.name)){const info=await stat(path);out.push({path:rel,name:entry.name,section:rel.split("/")[0],size:info.size,updatedAt:info.mtime.toISOString()});}}} await walk(base,"",0);return out.sort((a,b)=>b.updatedAt.localeCompare(a.updatedAt)); }
export async function getEligibleBacklogJobs(agentId?:string){const jobs=await listJobs({status:"backlog",...(agentId?{assignee:agentId}:{})});return jobs.filter(j=>!j.indicators.blocked&&!j.indicators.needsOwnerInput&&!j.indicators.waitingForSubtask).sort((a,b)=>a.priority-b.priority||a.createdAt.localeCompare(b.createdAt));}
export async function findJobByExecution(executionId:string){for(const job of await listJobs()){try{const links=await getJobExecutions(job.projectSlug,job.id);const execution=links.executions.find(e=>e.executionId===executionId);if(execution)return {job,execution};}catch{}}return null;}
